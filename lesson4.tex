...
könnyen belátható, hogy \pr{fib2(I, K1, K2, M)}-re
mindig igaz lesz, hogy az \pr{N-I+1}-edik Fibonacci
szám a \pr{K1} lesz (\pr{I = N} esetén nyilvánvaló;
...
\end{problem}
\begin{problem}
\item Írjatok egy \pr{között(N1, N2, X)} szabályt,
...
A \pr{között} szabály feladatként volt bevezetve,
így definiálhatjuk:
\begin{program}
között(N, M, N) :- N =< M.
között(N, M, X) :-
...
többi lépés (aminél a \pr{PL}-et kiegészítjük az új
...
meg, amiből kocka készíthető? Ehhez a \pr{kígyó}-nak
