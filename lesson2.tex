...
\item Különleges karakterek (nem
  betű/szám/alsóvonás) sorozata, pl.~\pr{.:.},
  \pr{==>}, \pr{+}.
\item Aposztófok közti karaktersorozat,
  pl. \pr{'Peti'}, \pr{'Abú Tálib'}.
\end{enumerate}
...
érdekességekről rengeteget lehet olvasni a \emph{Zsargon
  fájl}\/ban,\footnote{http://www.catb.org/jargon/html/}
...
\pr{pont(X,Y,Z)} funktort is, illetve azonos nevű,
...
\begin{enumerate}
\item Ha \pr{S} és \pr{T} \emph{konstansok} (tehát
  atomok vagy számok), akkor csak abban az esetben
  egyesíthetőek, ha azonosak.\index{konstans}
\item Ha \pr{S} változó, akkor a két kifejezés
  egyesíthető, és innentől kezdve \pr{S} ,,értéke''
  \pr{T} lesz. (Fordítva hasonlóan.)
\item Ha \pr{S} és \pr{T} is struktúrák, akkor pontosan akkor egyesíthetőek, ha
  \begin{itemize}
